...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "JobStory":
		resolverName, err := entityResolverNameForJobStory(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "JobStory": %w`, err)
		}
		switch resolverName {
		...
		case "findJobStoryByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findJobStoryByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindJobStoryByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "JobStory": %w`, err)
			}
...
func entityResolverNameForJobStory(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for JobStory", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for JobStory", ErrTypeNotFound))
			break
		}
		return "findJobStoryByID", nil
	}
	return "", fmt.Errorf("%w for JobStory due to %v", ErrTypeNotFound,
